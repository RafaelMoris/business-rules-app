// index.js
var REST_DATA = 'question/createQuestion';
var KEY_ENTER = 13; // Enter Key
var defaultItems = [

];
var OPENED_ANSWER_KEY = 'opened-answer';
var CLOSED_ANSWER_KEY = 'closed-answer';
...
function addItem(item, isNew) {
    ...
    {
      var rowHTML = '<td class="contentName"><select id="question-type" onchange="toggleQuestionType(this)"><option value="' + OPENED_ANSWER_KEY + '">Resposta Aberta</option><option value="' + CLOSED_ANSWER_KEY + '">Resposta Fechada</option></select></td>';
      rowHTML += '<td class="contentName"><textarea id="question-text" onkeydown="onKey(event)" placeholder="Insira o texto da pergunta..."></textarea></td>';
      rowHTML += '<td class="contentDetails" id="question-answer-options"></td>';
      rowHTML += '<td class ="contentAction"><span class="deleteBtn" onclick="deleteItem(this)" title="delete me"></span></td>';
...
function toggleQuestionType(item) {
  var questionType = item.options[item.selectedIndex].getAttribute('value');

  var itemHTML;
  var row = $(item).parent().parent().find('#question-answer-options');
  if (questionType == OPENED_ANSWER_KEY) {
    itemHTML = "";
    row.html(itemHTML);
  }else {
    itemHTML = '<input type="checkbox" id="allow_multi_selection"> Permitir seleção multipla</br><table id="answer-options" class="records" border="0" cellspacing="0" cellpadding="0"><tbody></tbody></table>';
    itemHTML += '<button class="addBtn" onclick="addAnswer(this)" title="add answer"><img src="images/add.png" alt="add"></button>';
    row.html(itemHTML);
  }

}

function addAnswer(target, item, isNew) {
    var row = document.createElement('tr');
    row.className = "tableRows";
    var id = item && item.id;
    if (id) {
    ...
    {
      var rowHTML = '<td class="contentName"><textarea id="question-text" onkeydown="onKey(event)" placeholder="Insira uma espota..."></textarea></td>';
      rowHTML += '<td class ="contentAction"><span class="deleteBtn" onclick="deleteItem(this)" title="delete me"></span></td>';
      row.innerHTML = rowHTML;
    }

    var table = $(target).parent().find('#answer-options');
    table.append($(row)[0]);
...
function onKey(evt) {
    ...
    if (evt.keyCode == KEY_ENTER && !evt.shiftKey) {
        ...
        var question;
        var type = OPENED_ANSWER_KEY;
        var row;

        if (evt.target.id == "question-text") {
